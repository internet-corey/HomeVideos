"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.encryptedApiKey = exports.decrypt = void 0;
const crypto_1 = require("crypto");
const fs_1 = require("fs");
function keyGen() {
    crypto_1.generateKeyPair('rsa', {
        modulusLength: 4096,
        publicKeyEncoding: {
            type: 'spki',
            format: 'pem'
        },
        privateKeyEncoding: {
            type: 'pkcs8',
            format: 'pem',
            cipher: 'aes-256-cbc',
            passphrase: 'top secret'
        }
    }, (err, pub, prv) => {
        if (!err) {
            console.log(`pubkey: ${pub}\nprivkey: ${prv}`);
        }
        else {
            console.log(`Error: ${err}`);
        }
    });
}
function encrypt(message, pubKeyPath) {
    const buffer = Buffer.from(message);
    const pubKey = fs_1.readFileSync(pubKeyPath, 'utf8');
    const encryptedMessage = crypto_1.publicEncrypt(pubKey, buffer);
    return encryptedMessage.toString('base64');
}
function decrypt(message, prvKeyPath) {
    const prvKey = fs_1.readFileSync(prvKeyPath, 'utf8');
    const buffer = Buffer.from(message, 'base64');
    const decryptedMessage = crypto_1.privateDecrypt({
        key: prvKey.toString(),
        passphrase: '',
    }, buffer);
    return decryptedMessage.toString('utf8');
}
exports.decrypt = decrypt;
const encryptedApiKey = '';
exports.encryptedApiKey = encryptedApiKey;
